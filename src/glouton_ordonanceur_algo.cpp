...
public class ordonanceur {
	...
	reference_Instance instanciation_component(Instance i){
	    if (empty(reference_component_started.get(i))){
	      ...
	      reference_Instance instancied_component = m.initialize_component(i.getPath(), i.getname(), i.getSign(), i.getDescriptor());
	...
	/* ------------------------------------------------------------------------ */
	void waiting_end_of_instruction(Instruction i){
	...
	/* ------------------------------------------------------------------------ */
	main (){
		assembly internal_assembly = cin << path; //On determine l'assembly qu'on veux utiliser
